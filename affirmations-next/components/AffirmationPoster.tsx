...
interface AffirmationPosterProps {
  initialData: AffirmationData
}

export default function AffirmationPoster({ initialData }: AffirmationPosterProps) {
  ...
  const handleGetNewOne = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch affirmation')
    } finally {
      setIsLoading(false)
    }
  }
